...
function GenerateHTML (data) {
    ...
    for(let i = 0; i < data.storage.length; i++) {
        ...
        if(employeeRole === 'Engineer') {
            employeeType = new Engineer (data.storage[i].name, data.storage[i].id, data.storage[i].email, data.storage[i].github);
        } else if (employeeRole === 'Manager') {
            employeeType = new Manager (data.storage[i].name, data.storage[i].id, data.storage[i].email, data.storage[i].office);
        } else {
            employeeType = new Intern (data.storage[i].name, data.storage[i].id, data.storage[i].email, data.storage[i].school);
        }
        // applies HTML to employees 
        if(employeeType.role === 'Manager') {
            htmlString[i] = `
            <div class="card text-light bg-primary m-3" style="width: 20rem; height: 14rem;">
            ...
            <div class="card text-light bg-primary m-3" style="width: 20rem; height: 14rem;">
                ...
                <div class="card-body">
                    ...
                    <p class="card-text">ID: ${employeeType.id}</p>
                    <p class="card-text"><a href="mailto:${employeeType.email}">Email: ${employeeType.email}</a></p>
                    <p class="card-text"><a href="https://github.com/${employeeType.github}" target="_blank">Github: ${employeeType.github}</a></p>
                </div>
            </div>`;
        };
    };
